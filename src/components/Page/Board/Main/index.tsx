...
export default function Main() {
  ...
  const { data, isLoading } = useQuery(["board", id], async () => {
    // @ts-ignore
  ...
  return (
    <main
      style={{
        background: `linear-gradient(225deg, #fff 0%, ${
          board ? board.backgroundURL : "#60a5fa"
        } 100%)`,
      }}
    ...
    >
      <section className="w-screen h-[calc(100vh-7rem-4rem)] flex flex-col px-6">
        <div className="flex flex-row items-center justify-start py-7 gap-x-4">
          <div className="min-w-[530px] flex flex-col">
            ...
            />
            {titleError && (
              <span className="text-notification-warning text-about">
