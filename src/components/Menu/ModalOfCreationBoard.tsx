...
const ModalOfBoardCreationBoard = ({ children }: ModalProps) => {
  ...
  const handleCreateBoard = useMutation(async (data: FormData) => {
  ...
  const handleCreateAndRedirectToBoard = async (values: FormData) => {
    const boardId = await handleCreateBoard.mutateAsync(values);
    reset();
    router.push(`/board/${boardId._id}`);
  };
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="data-[state=open]:animate-contentShow fixed top-[50%] left-[50%] max-h-[85vh] pb-4 w-full max-w-[460px] translate-x-[-50%] translate-y-[-50%] rounded-[6px] bg-[#FBFBFB] shadow-[hsl(206_22%_7%_/_35%)_0px_10px_38px_-10px,_hsl(206_22%_7%_/_20%)_0px_10px_20px_-15px] focus:outline-none">
          <Dialog.Title
            ...
            className={`text-mauve12 m-0 text-[17px] flex flex-row items-center justify-center font-medium w-full h-[118px] rounded-t-md gap-x-1`}
          >
            <label
              htmlFor="color-input"
              className="flex items-center justify-center bg-transparent cursor-pointer text-regular text-white leading-4 text-sm text-shadow gap-x-1"
            >
              <PaintBrushIcon />
              Update cover color
            </label>
          ...
          <form onSubmit={handleSubmit(handleCreateAndRedirectToBoard)}>
...
export default ModalOfBoardCreationBoard;
